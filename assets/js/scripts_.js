...
/* мои добавления*/
/*страница добавления коллбеков. подгружает результаты обзвона/встречи*/
 function load_r_call(select){
	var resultSelect = $('select[name="result_call"]');
	...
	$.getJSON('elements/ajax.php', {action:'getresultcall', type:select.value}, function(resultList){
...
/* ajax запросы на отключение/включение учетки. например*/
function disable_user(action,id){

	//$('#table').on('click','.disable', function (e) {
	//	e.preventDefault();
		
		/*if (confirm("Вы действительно хотите отключить данную запись?") == false) {
			return;
		}
			*/
	...
	//alert ($(this).parents('tr')[0]);
		var url = "elements/ajax.php?action="+action+"&id="+id;


			$.get(url, function (data) {
				if (data == "success") {
					if (action == "disableuser") {
						_toastr("Учетная запись отключена", "top-right", "info", false);
						//$(this).parents('tr').children('#status').text('<span class="label label-sm label-danger">Отключен</span>');
						//	$("class.disable").empty();
						//this.html('<a class="disable" href="#" onclick="disable_user(\'enableuser\,"+id+")">Включить </a>');
					//	$(r.parents('tr').children('#disable')).empty();
					//	r.parents('tr').children('#disable').append('<i class="fa fa-ban"></i> <a class="disable" href="#" onclick="disable_user(\'enableuser\',' + id + ')">Включить </a>');
						$("#status_"+id).empty();
						$('<span class="label label-sm label-danger">Отключен</span>').appendTo('#status_'+id);
						$("#disable_"+id).empty();
						$('<i class="fa fa-ban"></i> <a class="disable" href="#" onclick="disable_user(\'enableuser\','+id+')">Включить </a>').appendTo('#disable_'+id);

					}
					if (action == "enableuser") {
						_toastr("Учетная запись включена", "top-right", "info", false);
						$("#status_"+id).empty();
						$('<span class="label label-sm label-success">Включен</span>').appendTo('#status_'+id);
						$("#disable_"+id).empty();
						$('<i class="fa fa-ban"></i> <a class="disable" href="#" onclick="disable_user(\'disableuser\','+id+')">Отключить </a>').appendTo('#disable_'+id);
					//	$(this).parents('tr').children('#status').text('<span class="label label-sm label-success">Включен</span>');
					//		$("class.disable").empty();
					//	$(r.parents('tr').children('#disable')).empty();
					//	r.parents('tr').children('#disable').append('<i class="fa fa-ban"></i> <a class="disable" href="#" onclick="disable_user(\'disableuser\',' + id + ')">Отключить </a>');
					//	$(this).html('<a class="disable" href="#" onclick="disable_user(\'disableuser\',"+id+")">Отключить </a>');
