...
if ($CURUSER) {
	...
	if (!$id) {$content .="<h1>Нет опросов!</h1>".((get_user_class() >= ADMINISTARTOR)?"[<a href=\"".$REL_SEO->make_link('pollsadmin','action','add')."\">Создать новый</a>]":""); } else {
		...
		<ul class="polls_title_q">
			<li class="colheadli"><h3 style="margin-top: 7px;margin-bottom:0;">'.$pquestion.'</h3>'.((get_user_class() >= ADMINISTRATOR)?" <span style=\"margin-left: 335px;\">[<a href=\"".$REL_SEO->make_link('pollsadmin','action','add')."\">Создать новый</a>] [<a href=\"".$REL_SEO->make_link('pollsadmin','action','edit','id',$id)."\">Редактировать</a>] [<a href=\"".$REL_SEO->make_link('pollsadmin','action','delete','id',$id)."\" onClick=\"return confirm('Вы уверены?');\">Удалить</a>]":"").'</span></li>
		</ul>';
		...
		foreach ($votesres as $votedrow => $votes) {
			...
			foreach($votes as $vote) {
				...
				$usercode[$votedrow] .= "<a href=\"".$REL_SEO->make_link('userdetails','id',$userid,'username',$user['username'])."\">".get_user_class_color($user['class'],$user['username'])."</a>".((get_user_class() >= MODERATOR)?" [<a onClick=\"return confirm('Удалить этот голос?')\" href=\"".$REL_SEO->make_link('polloverview','deletevote','','vid',$vid)."\">D</a>] ":" ");
