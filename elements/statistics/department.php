<?php
    ...
    function processing_stat_client($department,$begin_month,$last_month){
        $res_client_month = sql_query("
        SELECT  users.id,users.name, COUNT(*) as num , client.status
        FROM users
        LEFT JOIN client ON  client.manager = users.id
        WHERE client.department = ".$department." AND client.added > ".$begin_month." AND client.added < ".$last_month."  GROUP BY users.id, client.status
        ;");
    ...
    function processing_stat_contact($department,$begin_month,$last_month){
        $res_callback_last_month = sql_query("
          SELECT  users.id,users.name, COUNT(*) as num , callback.type_contact
          FROM users
          LEFT JOIN callback ON  callback.id_user = users.id
          WHERE users.department = ".$department." AND callback.added > ".$begin_month." AND callback.added < ".$last_month." GROUP BY users.id, callback.type_contact
