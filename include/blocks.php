...
function show_blocks($position) {
	global $CURUSER, $REL_CONFIG, $REL_CACHE, $REL_TPL, $orbital_blocks;
	...
	foreach ( $orbital_blocks as $block ) {
		...
		if ($blockfile != "") {
			if (file_exists ( ROOT_PATH."blocks/" . $blockfile . "" )) {
				...
				$contenta = "<center>Нет файла или контента для этого блока</center>";
			}
		} else	$contenta .= render_content($content);
		if (!$blocktitlea) $blocktitlea .= $blocktitle;

		if ($bidpos) {
			$blocktitlea .= "&nbsp;<span style=\"cursor: pointer;\" onclick=\"javascript: showshides('b" . $bid . "');\"><img border=\"0\" src=\"img/plus.gif\" id=\"picb" . $bid . "\" title=\"Показать\"/></span>";
		} else {
			$blocktitlea .= "&nbsp;<span style=\"cursor: pointer;\" onclick=\"javascript: showshides('b" . $bid . "');\"><img border=\"0\" src=\"img/minus.gif\" id=\"picb" . $bid . "\" title=\"Скрыть\"/></span>";
		}
		...
		if ($block['custom_tpl']) 
			$REL_TPL->output($block['custom_tpl'],"blocks");
		else 
			$REL_TPL->display('block-'.$bposition.'.tpl');
